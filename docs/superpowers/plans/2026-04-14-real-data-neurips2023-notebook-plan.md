# real_data_neurips2023 Notebook Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `experiments/real_data_neurips2023.ipynb` — a notebook that reproduces the NeurIPS 2023 Table 2 / Figure 2 results using the `NEURIPS2023` named collections and `RidgeEM(t2=False)`.

**Architecture:** Single new notebook. All code cells tagged `skip-execution`. Imports deferred to first cell that needs each name. Figure code replicated from `real_data.ipynb` (cell 11). No preview experiment, no separate large-dataset section.

**Tech Stack:** `experiments/real_data_neurips2023.ipynb`, nbformat 4.5.
...
Write the following JSON to `experiments/real_data_neurips2023.ipynb`. Every code cell has `"tags": ["skip-execution"]`. Cell IDs are short unique strings.
 ...
 "cells": [
  {
   ...
   "id": "a001",
   "metadata": {},
   "source": ["# Real Data — NeurIPS 2023 Reproduction\n",
               "\n",
...
- [ ] **Step 2: Verify the notebook is valid JSON and has the expected cell count**

```bash
cd /Users/marioboley/Documents/GitHub/fastridge && source .venv/bin/activate && python -c "
import json
nb = json.load(open('experiments/real_data_neurips2023.ipynb'))
print('cells:', len(nb['cells']))
print('nbformat:', nb['nbformat'], nb['nbformat_minor'])
for i, c in enumerate(nb['cells']):
    tags = c.get('metadata', {}).get('tags', [])
    src = ''.join(c['source'])[:60].replace('\n', ' ')
    print(f'  {i} [{c[\"cell_type\"]}] tags={tags}: {src}')
"
```

Expected: 14 cells (4 markdown + 10 code), all code cells have `tags: ['skip-execution']`.

- [ ] **Step 3: Run the full test suite to confirm existing tests still pass**

```bash
cd /Users/marioboley/Documents/GitHub/fastridge && source .venv/bin/activate && pytest 2>&1 | tail -5
```

Expected: all tests pass (the new notebook is not in pytest.ini so it is not executed by CI).

- [ ] **Step 4: Commit**

```bash
git add experiments/real_data_neurips2023.ipynb
git commit -m "feat: add real_data_neurips2023 notebook with NEURIPS2023 collections and t2=False"
```
